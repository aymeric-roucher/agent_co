  ...
  return {
    start_worker: tool({
      ...
      execute: async ({ task, branch_name }) => {
        ...
        // Set git author to the VP's identity so commits are attributed correctly
        const vpName = `${state.config.name} VP`;
        execSync(`git config user.name "${vpName}"`, { cwd: worktreePath, stdio: 'pipe' });
...
### Before/After — Concrete numbers if applicable (test count, coverage, etc.)`),
      ...
      execute: async ({ branch, title, description, images }) => {
        ...
        // Ensure the department label exists
        const label = `vp:${state.config.slug}`;
        try {
          execSync(`gh label create "${label}" --color 0E8A16 --description "PRs from ${state.config.name} VP" --force`, { cwd: repo, stdio: 'pipe' });
        } catch { /* label may already exist */ }

        const result = execSync(
          `gh pr create --head "${branch}" --title "${title}" --label "${label}" --body "$(cat <<'PRBODYEOF'\n${body}\nPRBODYEOF\n)"`,
