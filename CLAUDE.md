...
## Deliverable
- Single self-contained `weather-dashboard.html` at repo root.
- Inline CSS + JS only (no build system).
- Notional data for a handful of cities.

## Testing
- Repo uses Vitest (`npm test` / `vitest run`).
- Optional: `tests/weather-dashboard.test.ts` can read the HTML file and assert key strings (title, city names). Keep tests simple and not DOM-dependent.

## Git/Workers
- Some workers report inability to commit due to sandbox worktree `.git` index.lock permissions. VP should use `open_pr` tool which auto-commits and pushes from the branch.
