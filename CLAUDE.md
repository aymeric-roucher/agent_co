...
## Common repo/testing notes
- Repo appears to use **Vitest** (`npm test` runs `vitest run`). In some sandboxes, `vitest` is missing until `npm install` is run.
- If adding tests, keep them minimal and deterministic: read HTML file and assert it contains city names and basic structure.

## Operational notes
- Workers may be unable to `git add/commit` due to `.git/worktrees/.../index.lock` permission restrictions. VP should use `open_pr` tool to commit/push/create PR.

## File naming
- Some prompts mention `index.html`, others `weather-dashboard.html`. If ambiguous, either:
  1) Use `weather-dashboard.html` as primary, or
  2) Provide both with identical contents to satisfy either expectation.
